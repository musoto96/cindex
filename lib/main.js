  ...
  if (answer) {
    ...
    Promise.all(createPromises)
      .then(() => {
        ...
        fs.promises.writeFile(`${path}/${_config.dirs.templates}/sample_template.html`, sampleDraftTemplate);
        ...
        fs.promises.writeFile(`${path}/${_config.dirs.index}/_categories.html`, tagIndex("categories"));
        fs.promises.writeFile(`${path}/${_config.dirs.index}/_tags.html`, tagIndex("tags"));
        fs.promises.writeFile(`${path}/${_config.dirs.index}/_tags.json`, JSON.stringify({}));
...
async function generate(argPath, printInfo=true) {
  ...
  if (missing.length > 0) {
    ...
    for (const draft of missing) {
      ...
      // Create page
      const { name, page, tags } = createPage(pageTemplate, pageContent, "page");
      ...
      // Create a list of tags to consume
      let jsonTags = JSON.parse((await fs.promises.readFile(`${path}/${_config.dirs.index}/_tags.json`)).toString());

      let newTags;
      if (jsonTags.tags && jsonTags.tags.length > 0) {
        newTags = tags.filter(tag => !jsonTags.tags.includes(tag));
      } else {
        newTags = tags;
      }
      ...
      if (newTags && newTags.length > 0) {
        if (jsonTags.tags && jsonTags.tags.length > 0) {
          jsonTags.tags = [ ...jsonTags.tags, ...newTags ].sort();
        } else {
          jsonTags.tags = [ ...newTags ].sort();
        }

        // Update categories index and page
        for (const tag of tags) {
          // TODO
          const newCategoryIndex = await updateIndex(`${path}/${_config.dirs.index}`, name, `../${_config.dirs.pages}/${draft}`, fileDate + ", " + fileTime, tag, indexFile=`_${tag}.html`);

          const categoryIndex = createPage(pageTemplate, newCategoryIndex, "tag").page;
          await fs.promises.writeFile(`${path}/${_config.dirs.categories}/${tag}.html`, categoryIndex);
        }

        for (const tag of newTags) {

          // Update tag index
          const newTagIndex = await updateIndex(`${path}/${_config.dirs.index}`, tag, `${_config.dirs.categories}/${tag}.html`, fileDate + ", " + fileTime, tag, indexFile="_categories.html", useDate=false);
          const categoryIndexPage = createPage(indexPage, newTagIndex, "tag").page;
          await fs.promises.writeFile(`${path}/${_config.files.index}`, categoryIndexPage);
      ...
      await fs.promises.writeFile(`${path}/${_config.dirs.index}/_tags.json`, JSON.stringify(jsonTags, null, 2));
...
async function regenerate(argPath) {
  ...
  let drafts = await fs.promises.readdir(`${path}/${_config.dirs.drafts}`);
  drafts = drafts.map(fileName => ({
    name: fileName,
    time: fs.statSync(`${path}/${_config.dirs.drafts}/${fileName}`).mtime.getTime(),
  }))
