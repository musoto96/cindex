...
async function updateIndex(path, page, uri, date) {
  const indexLines = (await fs.promises.readFile(`${path}/_index.html`)).toString().split("\n");
  const index = indexLines.slice(0, indexLines.length-1).join("\n");
  const entry = `<li class="cindex-index">${date} - <a href="${uri}">${page}</a></li>`;
...
async function indexDel(pathIndex, uri) {
  ...
  return await fs.promises.writeFile(`${pathIndex}/_index.html`, newIndex);
}

async function indexReset(pathIndex) {
  const index = (await fs.promises.readFile(`${pathIndex}/_index.html`)).toString();
